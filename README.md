...
aggregates/latest.json          # Newest snapshot per network with its full breakdowns
```

The daily files are the data; everything under `aggregates/` is derived from them and
regenerated on every push, so build against `daily/` if you need a stable contract.
`latest.json` exists because the time-series files carry only counts — it is where the
country, ASN, user-agent and failure-reason breakdowns of the most recent round live.
